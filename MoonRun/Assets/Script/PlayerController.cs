...
public class PlayerController : MonoBehaviour {
    ...
    void GetInputDirection()
    {
        ...
        {
            Vector3 vec = Input.mousePosition - mousePos;//鼠标拖动距离
            if (vec.magnitude > 20)//向量长度，多用返回物体的速度，speed=rigidbody.velocity.magnitude
            {
                //计算夹角（Vector3.Dot计算两个向量夹角的余弦值，两个向量必须是单位向量）
                var angleY = Mathf.Acos(Vector3.Dot(vec.normalized, Vector2.up)) * Mathf.Rad2Deg;//弧度转度
